...
- **Ability type field values**: `"Movement"` = client-only (skipped server-side in `SharedWeaponBase._createAbilities`), `"Defense"` = `DefenseBaseAbility`, `"Attack"` = `AttackBaseAbility`, `"Range"` = `RangeAttack`/`RangeAttackClient`, `"Others"` = `MovementAbility` server-side. On client, only `"Range"` and `"Movement"` are instantiated.
...
1. **Client** — `src/client/UI/ServiceRoot.client.luau:138` listens on `player_character.ChildAdded`. If the new child is a `Tool` it fires the server (`ReplicatedStorage.RemoteEvents.Equipment`) and initialize animationHandler as armed via `characterCtxUtils:InitChar(character, tool.Name:lower())`.
2. **Server** — `src/server/Combat/CombatMediator.luau` `Equipment.OnServerEvent` handler does the same `characterCtxUtils:InitChar(char, r6Name)` on the server side.
3. **Unequip path** — currently underspecified. The client's `ChildAdded` branch does not have a matching `ChildRemoved` branch, so the server never receives `has_to_equip = false` from normal tool removal. Any new code that wants the strategy destroyed on unequip must either (a) fire the Equipment remote with `has_to_equip = false`, or (b) detect the swap inside `EquiTool` and destroy the previous `self._weapon` before assigning the new one. See the strategy-swap rule above.
